...
function isCurrentTimeMatch(currentTime: number, classStart: number, classEnd: number) {
  ...
  const start = Number(classStart);
  const end = Number(classEnd);

  if (end < start) {
    const adjustedEnd = end + 1440;
    let adjustedCurrent = curr;
    if (curr < start) adjustedCurrent += 1440;

    const result = {
      before10: adjustedCurrent === start - 10,
      before5: adjustedCurrent === start - 5,
      start: adjustedCurrent === start,
      end: adjustedCurrent === adjustedEnd,
    };
    console.log({
      label: '⏰ Cross-day time check',
      currentTime: curr,
      classStart: start,
      classEnd: end,
      adjustedCurrent,
  ...
  const result = {
    before10: curr === start - 10,
    before5: curr === start - 5,
    start: curr === start,
    end: curr === end,
  };
  console.log({
    label: '⏰ Normal time check',
    currentTime: curr,
    classStart: start,
    classEnd: end,
    result
